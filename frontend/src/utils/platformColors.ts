...
export type Platform = 'anthropic' | 'openai' | 'antigravity' | 'gemini' | 'kimi' | 'mimo' | 'qwen' | 'deepseek' | 'grok' | 'composite'
...
const BADGE: Record<Platform, string> = {
  ...
  kimi: 'bg-cyan-500/10 text-cyan-600 border-cyan-500/30 dark:text-cyan-400',
...
const BADGE_LIGHT: Record<Platform, string> = {
  ...
  kimi: 'bg-cyan-500/10 text-cyan-600 dark:bg-cyan-500/10 dark:text-cyan-300',
...
const ACCENT_BAR: Record<Platform, string> = {
  ...
  kimi: 'bg-gradient-to-r from-cyan-400 to-cyan-500',
...
const TEXT: Record<Platform, string> = {
  ...
  kimi: 'text-cyan-600 dark:text-cyan-400',
...
const BUTTON: Record<Platform, string> = {
  ...
  kimi: 'bg-cyan-500 text-white hover:bg-cyan-600 active:bg-cyan-700 dark:bg-cyan-500/80 dark:hover:bg-cyan-500',
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'antigravity' || p === 'gemini' || p === 'kimi' || p === 'mimo' || p === 'qwen' || p === 'deepseek' || p === 'grok' || p === 'composite'
}
