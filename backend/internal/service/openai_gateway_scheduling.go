...
func isOpenAICompatibleAccountEligibleForRequest(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	...
	} else if account.Platform != PlatformOpenAI && account.Platform != PlatformDeepSeek {
		return false
	}
	if account.IsOpenAI() {
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool) {
	...
	if fillModeEnabled {
		return sortOpenAIFillModeAccounts(eligible, requireCompact)[0], compactBlocked
	}
...
func (s *OpenAIGatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string) ([]Account, error) {
	platform = normalizeOpenAICompatiblePlatform(platform)
	loadPlatformAccounts := func(targetPlatform string) ([]Account, error) {
		if s.schedulerSnapshot != nil && targetPlatform != PlatformDeepSeek {
			accounts, _, err := s.schedulerSnapshot.ListSchedulableAccounts(ctx, groupID, targetPlatform, false)
			return accounts, err
		}
		if s.accountRepo == nil {
			return nil, nil
		...
		if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
			return s.accountRepo.ListSchedulableByPlatform(ctx, targetPlatform)
		}
		if groupID != nil {
			return s.accountRepo.ListSchedulableByGroupIDAndPlatform(ctx, *groupID, targetPlatform)
		}
		return s.accountRepo.ListSchedulableUngroupedByPlatform(ctx, targetPlatform)
	}

	accounts, err := loadPlatformAccounts(platform)
	if err != nil {
		return nil, fmt.Errorf("query accounts failed: %w", err)
	}
	...
	deepSeekAccounts, deepSeekErr := loadPlatformAccounts(PlatformDeepSeek)
	if deepSeekErr != nil {
		slog.Warn("openai_gateway.deepseek_accounts_query_failed", "group_id", derefGroupID(groupID), "error", deepSeekErr)
	} else {
		accounts = append(accounts, deepSeekAccounts...)
	}
	return accounts, nil
}
