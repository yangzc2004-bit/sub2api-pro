	...
	for {
		...
		for {
			...
			if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
				...
				result, err = h.gatewayService.ForwardMimoMessages(requestCtx, c, account, attemptBody, attemptParsedReq)
			} else if account.Platform == service.PlatformQwen {
				// Qwen 平台：将 Anthropic 格式转换为 OpenAI Chat Completions 格式
				chatCompletionsBody, convErr := service.ConvertAnthropicToChatCompletions(attemptBody)
				if convErr != nil {
